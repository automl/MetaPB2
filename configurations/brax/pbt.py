...
from src.rl.environment_families import FAMILIES, G_PLANET, PLANET_GRAVITY
from src.utils import get_float_transformer, get_keep_bounds
...
scheduler_config_log = OrderedDict({
    ...
    "custom_explore_fn": get_keep_bounds({"lambda": [0.9, 0.99], "clip_param": [0.1, 0.5], "lr": [1e-5, 1e-3]}),
})
...
tune_config = OrderedDict({
    ...
    "mode": ["max"],
    "scheduler": [
        dict(type='PBT', **scheduler_config_log),
    ],
    "num_samples": [4],
})
