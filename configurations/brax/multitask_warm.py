...
def get_trial_name(experiment_config):
    ...
    trial_name = {'task_name': env, 'algorithm': f'{algorithm} warm'}
...
scheduler_config = OrderedDict({
    "time_attr": "timesteps_total",
    "perturbation_interval": 200_000,
    "hyperparam_bounds": {
            "lambda": [0.9, 0.99],
            "clip_param": [0.1, 0.5],
            "lr": [1e-5, 1e-3],
    ...
    "meta_directory": "meta_data/brax/pb2",
    "use_hp_initializer": True,
    "meta_kwargs": {"loss_function": 'ranking', "all_data": True, "fraction": 1},
    "kernel_kwargs": {"kernel_type": "lcm", "n_points": 100, 'top_k': 4}
})
...
tune_config = OrderedDict({
    ...
    "mode": ["max"],
    "scheduler": [
        dict(type='MultiTaskPB2', **scheduler_config),
    ],
    "num_samples": [4],
})
...
run_config = OrderedDict({
    "stop": [{"timesteps_total": 3_000_000}],
    "verbose": [4],
    "log_to_file": [True],
