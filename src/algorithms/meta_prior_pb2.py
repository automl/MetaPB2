...
from src.algorithms.algorithm_utils import get_indices_to_logtr, process_data, get_fixed_UCB, timing
...
def prior_prediction(X, meta_data, weights, min_target_T, max_target_T):
    y_prior = np.zeros((X.shape[0], 1))
    # weights sum to 1 so we don't have to normalize by their sum
    for idx, model_data in enumerate(meta_data.meta_models):
        if weights[idx] == 0.:
            continue
        model = model_data['model']
        X_meta_model = meta_data.target_input_to_meta(X, min_target_T, max_target_T, idx)
        mean, _ = model.predict(X_meta_model)
        y_prior += meta_data.scale_output(mean, 0., min_target_T, max_target_T, idx)[0] * weights[idx]
    return y_prior


class GPPriorWrapper:
    def __init__(self, model, meta_data, weights, min_T, max_T, y_mean, y_std):
        if min_T > max_T:
            raise ValueError(f'min_T must be smaller or equal max_T')
        self.X = model.X  # this is not the best solution. Maybe I should inherit from GP or somehow extend it?
        self.model = model
        self.weights = weights
        self.T_range = [min_T, max_T]
        self.y_mean = y_mean
        self.y_std = y_std
    ...
    def predict(self, X):
        ...
        y_prior = prior_prediction(X, self.meta_data, self.weights, self.T_range[0], self.T_range[1])
        mean = mean * (self.y_std + 1e-8) + self.y_mean + y_prior
        var = var * (self.y_std + 1e-8) ** 2
        return mean, var
...
def select_config(
    ...
    Args:
        ...
        meta_data: The meta data object containing the meta models.
        ...
        gp_statistics: A dictionary containing some statistics of the GP.
        kernel_kwargs: The kernel kwargs for the GP.
        acquisition_kwargs: The acquisition kwargs for the GP.
    ...
    # enough datapoints
    if X.shape[0] > 3:  # ToDo: stop using the meta data after some point
        num_samples = 100
        weights = meta_data.compute_rank_weights(X, y, Xraw[:, 0], False, num_samples)

        min_target_T = np.min(Xraw[:, 0])
        max_target_T = np.max(Xraw[:, 0])
        y_prior = prior_prediction(X, meta_data, weights, min_target_T, max_target_T)
        y_diff = y - y_prior
        y_mean = np.mean(y_diff, axis=0)
        y_std = np.std(y_diff, axis=0)
        y = (y_diff - y_mean) / (y_std + 1e-8)
        m, m1 = pb2_gp(X, y, current, kernel_kwargs)
        m = GPPriorWrapper(m, meta_data, weights, min_target_T, max_target_T, y_mean, y_std)
        m1 = GPPriorWrapper(m1, meta_data, weights, min_target_T, max_target_T, y_mean, y_std)
    else:
        m, m1 = pb2_gp(X, y, current, kernel_kwargs)
    ...
    if hasattr(m, 'kern'):
        ...
        update_gp_statistics(gp_statistics, m.model, newpoint, bounds)
...
class MetaPriorPB2(LogPB2):
    """Implements the MetaPriorPB2 algorithm.

    This algorithm is a variant of PB2 that uses an ensemble of meta models as a prior for the GPs mean.
    ...
    def __init__(
        ...
        super(MetaPriorPB2, self).__init__(
            time_attr=time_attr,
            metric=metric,
            mode=mode,
